...
size_t part1(size_t steps = 100, bool returnAllFlashed = false) {
    ...
    const auto increment = [&grid](size_t r, size_t c, const std::vector<std::vector<bool>>& flashed) {
    ...
    const auto flash = [&grid, &increment](size_t r, size_t c, std::vector<std::vector<bool>>& flashed) {
        flashed[r][c] = true;
        grid[r][c] = 0;
    ...
    const auto countFlashes = [](const std::vector<std::vector<bool>>& flashed) {
        ...
        return std::accumulate(flashed.begin(), flashed.end(), 0UL, [](const auto& sum, const auto& row) {
            return sum + std::accumulate(row.begin(), row.end(), 0UL, [](const auto& sumRow, const auto& f) {
                       return sumRow + f;
                   });
        });
    };
    ...
    const auto allFlashed = [&grid]() {
        return std::all_of(grid.begin(), grid.end(), [](const auto& row) {
            return std::all_of(row.begin(), row.end(), [](const auto& e) {
                return e == 0;
            });
        });
    };
