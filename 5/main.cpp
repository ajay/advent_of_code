...
size_t pointMax(const std::vector<std::pair<std::pair<size_t, size_t>, std::pair<size_t, size_t>>>& points) {
    size_t max = 0;
...
size_t part1(bool diagonals = false) {
    ...
    return std::accumulate(map.begin(), map.end(), 0UL, [](const size_t& sum, const auto& row) {
        return sum + static_cast<size_t>(std::count_if(row.begin(), row.end(), [](const auto& e) {
                   return e > 1;
               }));
    });
}
