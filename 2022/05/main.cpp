...
std::pair<std::vector<std::vector<char>>, std::vector<std::tuple<size_t, size_t, size_t>>> parseFile(
    bool example = false) {
    ...
    const auto& stackBottom = *std::next(stackLines.rbegin());  // bottom of stacks, assume each stack has at least 1
    const auto numStacks = static_cast<size_t>(std::count_if(stackBottom.begin(), stackBottom.end(), [](const auto& c) {
        return std::isalpha(c);
    }));
    std::vector<std::vector<char>> stacks(numStacks);

    // skip last line (stack indexes), second last to first
    std::for_each(std::next(stackLines.rbegin()), stackLines.rend(), [&stacks, &numStacks](const auto& line) {
    ...
    std::transform(procedureLines.begin(), procedureLines.end(), std::back_inserter(procedure), [](const auto& line) {
...
std::string topOfStacks(bool example, bool moveTogether) {
    ...
    std::string top{};
    std::transform(stacks.begin(), stacks.end(), std::back_inserter(top), [](const auto& stack) {
        return stack.back();
    });
