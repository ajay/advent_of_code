...
struct Directory {
    ...
    // if too slow, memoize
    size_t size() const {
        const auto fileSize = std::accumulate(files.begin(), files.end(), 0UL, [](const auto& sum, const auto& file) {
            const auto& [_name, size] = file;
            return sum + size;
        });

        return std::accumulate(dirs.begin(), dirs.end(), fileSize, [](const auto sum, const auto& dir) {
            return sum + dir.size();
        });
    }
    ...
    size_t part1(size_t maxSize = 100000) const {
        ...
        return std::accumulate(dirs.begin(), dirs.end(), partialSum, [](const auto sum, const auto& dir) {
            return sum + dir.part1();
        });
    }
    ...
    while (line < terminal.size()) {
        ...
        if (terminal[line].rfind("$ cd", 0) == 0) {
            ...
            if (name == "..") {
                ...
                const auto it = std::find_if(dirs.begin(), dirs.end(), [&name](const auto& dir) {
