...
std::optional<std::vector<Point>> aStar(const Point& start, const Point& finish,
    ...
    while (openSet.size()) {
        const auto current =
            *std::min_element(openSet.begin(), openSet.end(), [&getFScore](const auto& p1, const auto& p2) {
...
std::optional<std::vector<Point>> runAStar(const std::vector<std::vector<size_t>>& grid, const Point& start,
                                           const Point& finish) {
    const auto heuristic = [](const Point& current, const Point& goal) {
        ...
        const size_t dx = static_cast<size_t>(std::abs(static_cast<ssize_t>(x1) - static_cast<ssize_t>(x2)));
        const size_t dy = static_cast<size_t>(std::abs(static_cast<ssize_t>(y1) - static_cast<ssize_t>(y2)));
...
size_t part1(const std::vector<std::vector<size_t>>& grid = parseFile()) {
    ...
    size_t risk = 0;
    if (path && (path->size() > 1)) {
        risk = std::accumulate(std::next(path->begin()), path->end(), 0UL, [&grid](const auto& sum, const auto& point) {
            const auto& [x, y] = point;
            return sum + grid[x][y];
        });
    }
